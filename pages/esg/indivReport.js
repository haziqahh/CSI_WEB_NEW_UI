...
function HomePage() {
  ...
  const getQuestion = () => {
    APIHelpers.GET(
      "v1/questions?questionSetId=" + completedAssessment.questionSetID
    )
      .then((res) => {
  ...
  const getRecommeded = () => {
    ...
    if (
      lowest[0].score === lowest[1].score ||
      lowest[1].score === scoreRank[2].score
    ) {
    ...
    if (lowest.length === 3) {
      if (
        (lowest[0].score === lowest[1].score &&
          lowest[1].score === lowest[2].score) ||
        (lowest[0].score === lowest[1].score &&
          lowest[2].score === scoreRank[3].score) ||
        (lowest[1].score === lowest[2].score &&
          lowest[2].score === scoreRank[3].score)
      ) {
    ...
    if (lowest.length === 3) {
      lowest.map((val) => {
        let list = learningResources.filter(
          (item) => item.indicator === val.dimension
        );
      ...
      lowest.map((val, index) => {
        ...
        if (val.dimension === "ESG") {
          ...
          if (index === 0) {
            ...
            if (
              val.score !== lowest[index + 1].score &&
              val.score !== lowest[index - 1].score
            ) {
              random2 = Math.floor(Math.random() * (list.length - 1));
              do {
  ...
  const getAllAssessment = () => {
    APIHelpers.GET("v1/assessments")
      .then((res) => {
        if (res.items !== null) {
          let all = res.items.filter(
            (item) =>
              item.completionDate !== "0001-01-01T00:00:00Z" &&
              fullCompany.includes(item.smeID)
          );
  ...
  const showQuestions = (value) => (
    <Row className="flex flex-col">
      <p className="font-semibold text-xl 1400:text-lg 820:text-base text-darkGreen my-4">
        {value}
      </p>
      {question[value].map((val) => (
        <div className="py-2 ml-2">
          <div className="flex gap-x-2">
            <p className="text-lg 1400:text-base 820:text-sm font-semibold mb-2">
              {val.questionNo}.
            </p>
            <p className="text-lg 1400:text-base 820:text-sm font-semibold mb-2">
              {val.questionLabel}
          ...
          (val.questionType === "UPLOAD" && val.options.length < 3) ? (
            <p className="text-base 1400:text-sm 820:text-xs">
              ...
              <span className="font-bold">
                {allAnswer[val.questionNo - 1] === false ? "No" : "Yes"}
              </span>
            </p>
          ) : (
            <div>
              <p className="text-base 1400:text-sm 820:text-xs">Your Answer:</p>
              {allAnswer[val.questionNo - 1].length > 0 ? (
                allAnswer[val.questionNo - 1].map((val1) => (
                  <p className="font-bold">{val1}</p>
                ))
              ) : (
          ...
          (val.questionType === "UPLOAD" && val.options.length < 3) ? (
            <div className="flex gap-x-4">
              ...
              <p className="text-base 1400:text-sm 820:text-xs">
                YES{" "}
                {overallAnswer[val.id] !== 0
                  ? Math.round((overallAnswer[val.id] / assessmentNo) * 100)
                  : 0}
                %
              </p>
              <p className="text-base 1400:text-sm 820:text-xs">
                NO{" "}
                {assessmentNo - overallAnswer[val.id] !== 0
                  ? Math.round(
                      ((assessmentNo - overallAnswer[val.id]) / assessmentNo) *
                        100
                    )
                  : 0}
                %
      ...
      <Content className="bg-white px-12 min-h-9/10 w-full pb-16">
        <Row className="mt-8 min-h-8/10 gap-x-8 w-3/5 1400:w-4/5 820:w-full mx-auto">
          ...
          <Row className="w-full flex flex-col">
            <p className="text-darkGreen font-bold text-4xl 1400:text-3xl 820:text-2xl">
          ...
          <Row className="w-full mt-4 flex justify-between">
            <Col className="w-1/2 bg-darkTeal text-white text-base px-4 pb-8 pt-2">
              <Row className="bg-white text-darkTeal p-8 -ml-8 border-darkTeal border-4 flex flex-col">
                <Col>
                  <p className="text-xl 1400:text-lg 820:text-base">
                    Overall ESG Score
                  </p>
                </Col>
                <Col>
                  <p className="text-3xl 1400:text-2xl 820:text-xl mt-4 font-bold">
                    {score["overall"] + "%"}&emsp;
                    {getESGLevel(score["overall"])}
                  </p>
                </Col>
              </Row>
              <Row className="w-full mt-4">
                <p className="w-1/5">Company</p>:
                <p className="w-3/4 break-words ml-2">
                  {Object.keys(company).length > 0 ? company.companyName : ""}
                </p>
              </Row>
              <Row className="w-full mt-2">
                <p className="w-1/5">Industry</p>:
                <p className="w-3/4 break-words ml-2">
                  {Object.keys(company).length > 0 ? company.industry : ""}
                </p>
              </Row>
              <Row className="w-full mt-2">
                <p className="w-1/5">Assessment Date</p>:
                <p className="w-3/4 break-words ml-2">
                  {Object.keys(completedAssessment).length > 0
                    ? completedAssessment.formatCompletionDate
                    : ""}
                </p>
              </Row>
              <Row className="w-full mt-2">
                <p className="w-1/5">Report Validity</p>:
                <p className="w-3/4 break-words ml-2">
                  {Object.keys(completedAssessment).length > 0
                    ? completedAssessment.validityDate
                    : ""}
                </p>
              </Row>
            </Col>
            <Col className="w-2/5 bg-darkTeal text-white text-base px-4 pb-8 pt-2">
              <Row className="bg-white text-darkTeal px-8 py-5 -ml-8 border-darkTeal border-4 flex items-center justify-between">
                ...
                <p className="text-3xl 1400:text-xl 820:text-lg font-bold w-1/3 text-center">
                  {score["Environmental"] + "%"}
                </p>
                <p className="text-3xl 1400:text-xl 820:text-lg font-bold w-1/3">
                  {getESGLevel(score["Environmental"])}
                </p>
              </Row>
              ...
              <Row className="bg-white text-darkTeal px-8 py-2 -ml-8 border-darkTeal border-4 flex items-center justify-between mt-2">
                ...
                <p className="text-3xl 1400:text-xl 820:text-lg font-bold w-1/3">
                  {getESGLevel(score["Sustainable Procurement"])}
                </p>
              </Row>
            </Col>
          </Row>
          <Row className="w-full flex justify-between mt-8">
            <Col className="w-47%">
              <p className="text-darkGreen font-bold text-2xl 1400:text-xl 820:text-lg mt-8 mb-4">
                Recommended Learning Resources For {company?.companyName}
              </p>
              {recommended.length > 0
                ? recommended.map((val, index) => (
                    <a
                      className={
                        "flex items-center gap-x-4 rounded-full px-4 py-2 text-lg 1400:text-xl 820:text-lg text-white hover:text-white focus:text-white border-2 border-gray-300 bg-darkTeal mt-2"
                      }
                      href={val.link}
                      target="_blank"
                    >
                      <p className="bg-white rounded-full text-yellow-600 py-3 px-5 text-sm">
                        {index + 1}
                      </p>
              ...
              <div className="mt-2">
                <a
                  href="/esg/dashboard"
                  className="text-blue-400 text-lg 1400:text-base 820:text-sm hover:underline focus:underline"
                >
            ...
            <Col className="w-47%">
              <p className="text-darkGreen font-bold text-2xl 1400:text-xl 820:text-lg mt-8 mb-4">
                {company?.companyName} Comparison With Peers
              </p>
              {summary.length > 0 ? (
                <div
                  className={
                    window.screen.width > 1800 ? "flex justify-around" : ""
                  }
                >
                  <div className="flex justify-center -mx-8 -mb-8 mt-2">
                    {multiRadarChart(summary, window.screen.width * 0.3)}
                  </div>
                  <div
                    className={
                      window.screen.width > 1800
                        ? "flex flex-col gap-y-4 justify-center"
                        : "flex flex-col gap-y-4 justify-center mt-6"
                    }
                  >
            ...
            <Row className="w-full flex flex-col mt-8">
              ...
              {allDocName.length > 0
                ? allDocName.map((val, index) => (
                    <a
                      className={
                        "flex items-center gap-x-4 px-4 py-2 text-2xl 1400:text-xl 820:text-lg text-white hover:text-white focus:text-white border-2 border-gray-300 bg-darkTeal mt-2"
                      }
                      onClick={() => {
          ...
          {Object.keys(company).length > 0 &&
          Object.keys(allScore).length > 0 ? (
            <div className="my-8">
              <Row>
                <p className="text-darkGreen text-2xl 1400:text-xl 820:text-lg font-semibold">
                  Summary
                </p>
              </Row>
              <Row className="w-full flex justify-between mt-2">
                <Col className="w-[24.5%] bg-darkTeal rounded-tr-3xl rounded-bl-3xl px-4 py-8 text-base text-white">
                  ...
                  <p>
                    {company?.companyName} scored{" "}
                    {score["Environmental"] +
                      "% (" +
                      getESGLevel(score["Environmental"]) +
                      ")"}{" "}
                    in the Environmental Dimension which scores{" "}
                    {Math.abs(
                      score["Environmental"] - allScore?.Environmental
                    ) !== 0
                      ? Math.abs(
                          score["Environmental"] - allScore?.Environmental
                        ) + "%"
                      : null}{" "}
                    {score["Environmental"] - allScore?.Environmental > 0
                      ? "higher than"
                      : score["Environmental"] - allScore?.Environmental < 0
                      ? "lower than"
                      : "the same as"}{" "}
                ...
                <Col className="w-[24.5%] bg-darkTeal rounded-tl-3xl rounded-br-3xl px-4 py-8 text-base text-white">
                  ...
                  <p>
                    ...
                    {score["Social"] +
                      ...
                      getESGLevel(score["Social"]) +
                      ")"}{" "}
                    in the Social Dimension which scores{" "}
                    {Math.abs(score["Social"] - allScore?.Social) !== 0
                      ? Math.abs(score["Social"] - allScore?.Social) + "%"
                      : null}{" "}
                    {score["Social"] - allScore?.Social > 0
                      ? "higher than"
                      : score["Social"] - allScore?.Social < 0
                      ? "lower than"
                      : "the same as"}{" "}
                ...
                <Col className="w-[24.5%] bg-darkTeal rounded-tr-3xl rounded-bl-3xl px-4 py-8 text-base text-white">
                  ...
                  <p>
                    ...
                    {score["Governance"] +
                      ...
                      getESGLevel(score["Governance"]) +
                      ")"}{" "}
                    in the Governance Dimension which scores{" "}
                    {Math.abs(score["Governance"] - allScore?.Governance) !== 0
                      ? Math.abs(score["Governance"] - allScore?.Governance) +
                        "%"
                      : null}{" "}
                    {score["Governance"] - allScore?.Governance > 0
                      ? "higher than"
                      : score["Governance"] - allScore?.Governance < 0
                      ? "lower than"
                      : "the same as"}{" "}
                ...
                <Col className="w-[24.5%] bg-darkTeal rounded-tl-3xl rounded-br-3xl px-4 py-8 text-base text-white">
                  ...
                  <p>
                    ...
                    {score["Sustainable Procurement"] +
                      ...
                      getESGLevel(score["Sustainable Procurement"]) +
                      ")"}{" "}
                    in the Sustainable Procurement Dimension which scores{" "}
                    {Math.abs(
                      score["Sustainable Procurement"] -
                        allScore?.["Sustainable Procurement"]
                    ) !== 0
                      ? Math.abs(
                          score["Sustainable Procurement"] -
                            allScore?.["Sustainable Procurement"]
              ...
              <Row className="mt-2">
                ...
                <Col className="w-full bg-darkTeal rounded-tl-3xl rounded-br-3xl px-4 py-8 text-white text-base">
                  <p>
                    Overall {company?.companyName} garnered an ESG Maturity
                    Score of {Math.round(score["overall"])}%, which is a{" "}
                    {getESGLevel(score["overall"])} rating.
                  </p>
                </Col>
              </Row>
            </div>
          ...
          <Row className="flex flex-col">
            <div
              className="flex items-center mt-8 gap-x-2 cursor-pointer"
              onClick={() =>
                visibleDetails === true
            ...
            >
              {visibleDetails === false ? (
                <RightOutlined className="text-2xl 1400:text-xl 820:text-lg text-darkGreen" />
              ) : (
                <DownOutlined className="text-2xl 1400:text-xl 820:text-lg text-darkGreen" />
              )}
              <p className="text-darkGreen font-semibold text-2xl 1400:text-xl 820:text-lg">
                View Details
              </p>
            </div>
            {visibleDetails === true
              ? Object.keys(allAnswer).length > 0
                ? DIMENSION.map((val) => showQuestions(val))
                : null
              : null}
          </Row>
          <Row className="w-full justify-end text-darkTeal mt-8">
            <p>
              Report Generated: {date}&emsp;ESG Report Assessment ID:{" "}
              {completedAssessment.serialNo}
            </p>
          </Row>
          <Divider className="border-black mt-0" />
          <Row>
            <p className="font-medium text-justify mt-2 text-darkTeal">
              © {year} Centre For Sustainability Intelligence Sdn Bhd. CSI is
              ...
              site is provided “as is ” with no guarantee of completeness,
              ...
              will CSI, its related partnerships and corporations, or the
              directors, agents or employees thereof be liable to you or anyone
              else for any decision made or action taken in reliance on the
              information in this Report or any consequential, special or
